...
class Settings(BaseSettings):
    ...
    skill_library_v1: bool = True
    failure_ledger_v1: bool = True
    # Learning loop / A-B promotion are off by default; require explicit admin enable.
    skill_learning_v1: bool = False
    skill_ab_promotion_v1: bool = False

    # ─── Phase 3 — Operation Heavy Lifting ───────────────
    # All sandbox / supervisor-v2 paths default OFF until explicitly enabled.
    sandbox_execution_enabled: bool = False
    sandbox_worker_enabled: bool = False
    parallel_supervisor_v1: bool = False
    sandbox_promotion_hints: bool = False
    # Optional in-sandbox LLM loop (v1 keeps cost predictable — no LLM inside container).
    sandbox_llm_enabled: bool = False
    # Advertise officecli capability to sandbox job specs (the runner tool ships
    # in the image regardless; this only governs backend advertisement).
    sandbox_officecli_enabled: bool = False
